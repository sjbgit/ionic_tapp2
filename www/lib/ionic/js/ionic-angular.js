 ...
 * browser behaves, an Ionic app is able to keep track of the previous views, the current views, and
 * the forward views (if there is one).  However, a typical web browser only keeps track of one
  ...
  return {
    ...
    register: function(parentScope, viewLocals) {
      ...
      if (forcedNav) {
        ...
        // they went back one, set the old current views as a forward views
        ...
        // they went to the forward one, set the forward views to no longer a forward views
        ...
        if (forwardView.historyId && tmp.scope) {
          // if a history has already been created by the forward views then make sure it stays the same
        ...
        // if switching to a different history, and the history of the views we're switching
        // to has an existing back views from a different history than itself, then
        // it's back views would be better represented using the current views as its back views
        ...
        if (currentView) {
          // set the forward views if there is a current views (ie: if its not the first views)
          currentView.forwardViewId = viewId;

          action = ACTION_NEW_VIEW;

          // check if there is a new forward views within the same history
          if (forwardView && currentView.stateId !== forwardView.stateId &&
             currentView.historyId === forwardView.historyId) {
            // they navigated to a new views but the stack already has a forward views
            // since its a new views remove any forwards that existed
          ...
          // there's no current views, so this must be the initial views
        ...
        // add the new views to this history's stack
     ...
     * @description The app's views history data, such as all the views and histories, along
     * with how they are ordered and linked together within the navigation stack.
     * @returns {object} Returns an object containing the apps views history data.
     */
     ...
     * @description The app's current views.
     * @returns {object} Returns the current views.
     */
     ...
     * @description The ID of the history stack which is the parent container of the current views.
     ...
     * @description Gets and sets the current views's title.
     * @param {string=} val The title to update the current views with.
     * @returns {string} Returns the current views's title.
     */
     ...
     * @description Returns the views that was before the current views in the history stack.
     * If the user navigated from View A to View B, then View A would be the back views, and
     * View B would be the current views.
     * @returns {object} Returns the back views.
     */
     ...
     * @description Gets the back views's title.
     * @returns {string} Returns the back views's title.
     */
     ...
     * @description Returns the views that was in front of the current views in the history stack.
     * A forward views would exist if the user navigated from View A to View B, then
     * navigated back to View A. At this point then View B would be the forward views, and View
     * A would be the current views.
     * @returns {object} Returns the forward views.
     */
     ...
     * @description Navigates the app to the back views, if a back views exists.
     */
    goBack: function() {
     ...
     * @description Clears out the app's entire history, except for the current views.
     */
     ...
     * This both removes the views element from the DOM, and destroy it's scope.
     */
    clearCache: function() {
     ...
     * @description Sets options for the next views. This method can be useful to override
     * certain views/transition defaults right before a views transition happens. For example,
     * the {@link ionic.directive:menuClose} directive uses this method internally to ensure
     * an animated views transition does not happen when a side menu is open, and also sets
     * the next views as the root of its history stack. After the transition these options
     * are set back to null.
     *
     * Available options:
     *
     * * `disableAnimate`: Do not animate the next transition.
     * * `disableBack`: The next views should forget its back views, and set it to null.
     * * `historyRoot`: The next views should become the root views in its history stack.
     *
     * ```js
...
function($rootScope, $state, $location, $document, $ionicPlatform, $ionicHistory) {
  ...
  $rootScope.$on('$ionicHistory.change', function(e, data) {
    ...
    if (hist && hist.cursor > -1 && hist.cursor < hist.stack.length) {
      ...
      // go to it's last views in its stack
  ...
  // Set the document title when a new views is shown
  ...
  function onHardwareBackButton(e) {
    ...
    if (backView) {
      // there is a back views, go to it
      backView.go();
    } else {
      // there is no back views, so close the app instead
 ...
 * iOS will move forward by transitioning the entering views from right to center and the leaving
 * views from center to left. However, Android will transition with the entering views going from
 * bottom to center, covering the previous views, which remains stationary. It should be noted
 ...
 *
 * @param {string} transition Which style of views transitioning to use.
 *
 ...
 * @description  Maximum number of views elements to cache in the DOM. When the max number is
 * exceeded, the views with the longest time period since it was accessed is removed. Views that
 * stay in the DOM cache the views's scope, current state, and scroll position. The scope is
 * disconnected from the `$watch` cycle when it is cached and reconnected when it enters again.
 * When the maximum cache is `0`, the leaving views's element will be removed from the DOM after
 * each views transition, and the next time the same views is shown, it will have to re-compile,
 ...
 * @returns {number} How many views Ionic will hold onto until the a views is removed.
 */
 ...
 * navigate forward to the same views again, it'll create a new DOM element and controller
 ...
 *
 * The Modal is a content pane that can go over the user's main views
 * temporarily.  Usually used for making a choice or editing an item.
 *
 * Put the content of the modal inside of an `<ion-modal-views>` element.
 *
 * **Notes:**
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $q, $log, $ionicClickBlock) {
  ...
  var ModalView = ionic.views.Modal.inherit({
    ...
    show: function(target) {
      ...
      // if modal was closed while the keyboard was up, reset scroll views on
...
.service('$ionicNavBarDelegate', ionic.DelegateService([
   ...
   * (if it exists and there is a previous views that can be navigated to).
  ...
  return {
    $get: ['$q', '$rootScope', function($q, $rootScope) {
      var self = {
         ...
         * close the actionsheet, but it should not also go back a page views
         * or close a modal which may be open.
         *
         * The priorities for the existing back button hooks are as follows:
         *   Return to previous views = 100
         *   Close side menu = 150
         *   Dismiss modal = 200
         ...
         * previous views' action, but not any of the other actions.
         * 
 ...
 *
 * The Popover is a views that floats above an app’s content. Popovers provide an
 * easy way to present or gather information from the user and are
 * commonly used in the following situations:
 *
 * - Show more info about the current views
 * - Select a commonly used tool or configuration
 * - Present a list of actions to perform inside one of your views
 *
 * Put the content of the popover inside of an `<ion-popover-views>` element.
 *
 * @usage
 * ```html
 * <p>
 ...
 *
 *   // .fromTemplate() method
 *   var template = '<ion-popover-views><ion-header-bar> <h1 class="title">My Popover Title</h1> </ion-header-bar> <ion-content> Hello! </ion-content></ion-popover-views>';
 *
...
.service('$ionicScrollDelegate', ionic.DelegateService([
   ...
   * @returns {object} The scroll position of this views, with the following properties:
   ...
   * @description Does not allow this scroll views to scroll either x or y.
   * @param {boolean=} shouldFreeze Should this scroll views be prevented from scrolling or not.
   * @returns {object} If the scroll views is being prevented from scrolling or not.
   */
  'freezeScroll',
  /**
   * @ngdoc method
...
function($timeout, $document, $q, $ionicClickBlock, $ionicConfig, $ionicNavBarDelegate) {
  ...
  var ionicViewSwitcher = {
    ...
    create: function(navViewCtrl, viewLocals, enteringView, leavingView, renderStart, renderEnd) {
      ...
      // loop through to see if the views is already in the navViewElement
      ...
      var switcher = {
        ...
        loadViewElements: function(registerData) {
          ...
          for (x = 0, l = viewElements.length; x < l; x++) {
            ...
            if (viewEle.data(DATA_ELE_IDENTIFIER) === enteringEleIdentifier) {
              // we found an existing element in the DOM that should be entering the views
        ...
        render: function(registerData, callback) {
          if (alreadyInDom) {
            ...
            // or the element has cache-views="false" attribute
            if (viewState(viewLocals).cache === false || viewState(viewLocals).cache === 'false' ||
                enteringEle.attr('cache-views') == 'false' || $ionicConfig.views.maxCache() === 0) {
          ...
          // update that this views was just accessed
        ...
        transition: function(direction, enableBack, allowAnimate) {
          ...
          cachedAttr(enteringEle.parent(), 'nav-views-transition', enteringData.transition);
          cachedAttr(enteringEle.parent(), 'nav-views-direction', enteringData.direction);
          ...
          function transitionComplete() {
            ...
            // each parent nav-views will update which views are active and cached
        ...
        cleanup: function(transData) {
          ...
          if (leavingEle && transData.direction == 'back' && !$ionicConfig.views.forwardCache()) {
            // if they just navigated back we can destroy the forward views
  ...
  function getTransitionData(viewLocals, enteringEle, direction, view) {
    // Priority
    // 1) attribute directive on the button/link to this views
    // 2) entering element's attribute
    // 3) entering views's $state config property
    // 4) views registration data
    // 5) global config
    // 6) fallback value

    var state = viewState(viewLocals);
    var viewTransition = nextTransition || cachedAttr(enteringEle, 'views-transition') || state.viewTransition || $ionicConfig.views.transition() || 'ios';
    var navBarTransition = $ionicConfig.navBar.transition();
    direction = nextDirection || cachedAttr(enteringEle, 'views-direction') || state.viewDirection || direction || 'none';
...
function($scope, $element, $attrs, $compile, $timeout, $ionicNavBarDelegate, $ionicConfig, $ionicHistory) {
    ...
    var headerBarInstance = {
      ...
      setItem: function(navBarItemEle, itemType) {
        ...
        if (navBarItemEle) {
          ...
          // there's a default button for this side and no views button
  ...
  self.update = function(viewData) {
    ...
    // update the nav bar items, depending if the views has their own or not
   ...
   * back views. If the back button is not possible, for example, the
   * first views in the stack, then this will not force the back button
   * to show.
   */
...
function($scope, $element, $attrs, $compile, $controller, $ionicNavBarDelegate, $ionicNavViewDelegate, $ionicHistory, $ionicViewSwitcher, $ionicConfig, $ionicScrollDelegate) {
  ...
  self.init = function() {
    ...
    // Find the details of the parent views directive (if any) and use it
    // to derive our own qualified views name, then hang our own details
  ...
  self.register = function(viewLocals) {
    ...
    // register that a views is coming in and get info on how it should transition
  ...
  self.render = function(registerData, viewLocals, enteringView, leavingView, renderStart, renderEnd) {
    // register the views and figure out where it lives in the various
    ...
    switcher.init(registerData, function() {
      // the views is now compiled, in the dom and linked, now lets transition the views.
      // this uses a callback incase THIS nav-views has a nested nav-views, and after the NESTED
      // nav-views links, the NESTED nav-views would update which direction THIS nav-views should use
  ...
  self.beforeEnter = function(transitionData) {
    if (isPrimary) {
      // only update this nav-views's nav-bar if this is the primary nav-views
   ...
   * example, the very first views in the navigation stack would not have a
   * back views, so the back button would be disabled.
   */
    ...
    function onRelease(ev) {
      if (isPrimary && viewTransition && dragPoints && dragPoints.length > 1) {
        ...
        if (isSwipingRight && (releaseSwipeCompletion > 0.5 || velocity > 0.1)) {
          // complete views transition on release
          ...
          // cancel views transition on release
...
.controller('$ionicRefresher', [
  ...
  function($scope, $attrs,  $element, $ionicBind, $timeout) {
    ...
    function nativescroll(target, newScrollTop) {
      // creates a scroll event that bubbles, can be cancelled, and with its views
...
function($scope, $attrs, $ionicSideMenuDelegate, $ionicPlatform, $ionicBody, $ionicHistory, $ionicScrollDelegate) {
  ...
  /**
   * Set the content views controller if not passed in the constructor options.
   *
...
function($scope, $element, $attrs, $compile, $rootScope, $ionicViewSwitcher) {
  ...
  var deregIonNavBarInit = $scope.$on('ionNavBar.init', function(ev, delegateHandle) {
    // this views has its own ion-nav-bar, remember the navBarDelegateHandle for this views
    ev.stopPropagation();
  ...
  self.init = function() {
    ...
    // add listeners for when this views changes
  ...
  self.beforeEnter = function(ev, transData) {
    // this event was emitted, starting at intial ion-views, then bubbles up
    // only the first ion-views should do something with it, parent ion-views should ignore
  ...
  function generateNavBarItem(html) {
    if (html) {
      // every time a views enters we need to recreate its views buttons if they exist
...
function CollectionRepeatDirective($ionicCollectionManager, $parse, $window, $$rAF, $rootScope, $timeout) {
  ...
  function postLink(scope, element, attr, scrollCtrl, transclude) {
    ...
    function initAfterItemsContainer() {
      ...
      // Put everything in the views after the repeater into a container.
      if (!container.length) {
...
function RepeatManagerFactory($rootScope, $window, $$rAF) {
  ...
  return function RepeatController(options) {
    ...
    // We say it's a grid views if we're either dynamic or not 100% width
    ...
    // views is a mix of list/grid methods + static/dynamic methods.
    ...
    function StaticViewType() {
      ...
      // static views always returns the same object for getDimensions, to avoid memory allocation
        ...
        // scrolling up
          ...
          for (i = oldRenderStartIndex; i >= 0; i--) {
            if ((dim = this.getDimensions(i)) && dim.primaryPos <= scrollValue) {
              // when grid views, make sure the render starts at the beginning of a row.
...
.directive('ionInfiniteScroll', ['$timeout', function($timeout) {
  return {
    ...
    link: function($scope, $element, $attrs, ctrls) {
      ...
      // if this views is not beneath a scrollCtrl, it can't be injected, proceed w/ native scrolling
      if (jsScrolling) {
 ...
 * the menu is open. Additionally, this directive will reset the entering views's
 ...
 * app will navigate back one views when the back button is clicked.  More advanced behavior is also
 ...
 *
 * We can add buttons depending on the currently visible views using
 ...
 *   </ion-nav-bar>
 *
 *   <!-- where the initial views template will be rendered -->
 *   <ion-nav-views>
 *     <ion-views>
 ...
 * views template the ability to specify which buttons should show in the nav bar,
 ...
 *
 * ***Note*** that `ion-nav-buttons` must be immediate descendants of the `ion-views` or
    ...
    compile: function(tElement, tAttrs) {
      ...
      return {
        pre: function($scope, $element, $attrs, navBarCtrl) {
          ...
          if (parentViewCtrl) {
            // if the parent is an ion-views, then these are ion-nav-buttons for JUST this ion-views
 ...
 *
 * @description
 * The direction which the nav views transition should animate. Available options
 ...
 * views the ability to specify its own custom title element, such as an image or any HTML,
 * rather than being text-only. Alternatively, text-only titles can be updated using the
 * `views-title` {@link ionic.directive:ionView} attribute.
 *
 * Note that `ion-nav-title` must be an immediate descendant of the `ion-views` or
 * `ion-nav-bar` element (basically don't wrap it in another div).
 *
 * @usage
 * ```html
 ...
 *
 * @description
 * The transition type which the nav views transition should use when it animates.
 ...
 * and remember to replace ui-views with ion-nav-views in examples).
 *
 * @usage
 * In this example, we will create a navigation views that contains our different states for the app.
 *
 ...
 * and then try to load home.html into the `<ion-nav-views>`.
 *
 ...
 *   <!-- The title of the ion-views will be shown on the navbar -->
 *   <ion-views views-title="Home">
 ...
 * ## Caching
 *
 * By default, views are cached to improve performance. When a views is navigated away from, its
 * element is left in the DOM, and its scope is disconnected from the `$watch` cycle. When
 * navigating to a views that is already cached, its scope is then reconnected, and the existing
 * element that was left in the DOM becomes the active views. This also allows for the scroll
 ...
 * needs to know when a views has entered or has left, then views events emitted from the
 ...
 * If you navigate forward to the same views again, it'll create a new DOM element and controller
 ...
 *
 * ```html
 * <ion-views cache-views="false" views-title="My Title!">
 *   ...
 * </ion-views>
 * ```
 *
 ...
 *
 * @param {string=} name A views name. The name should be unique amongst the other views in the
 * same state. You can have views of the same name that live in different states. For more
 * information, see ui-router's
 * [ui-views documentation](http://angular-ui.github.io/ui-router/site/#/api/ui.router.state.directive:ui-views).
 */
IonicModule
...
function($state, $ionicConfig) {
  ...
  return {
    ...
    compile: function(tElement, tAttrs, transclude) {
      ...
      // a nav views element is a container for numerous views
      tElement.addClass('views-container');
      ionic.DomUtil.cachedAttr(tElement, 'nav-views-transition', $ionicConfig.views.transition());

      return function($scope, $element, $attr, navViewCtrl) {
        var latestLocals;

        // Put in the compiled initial views
        ...
        function updateView(firstTime) {
          ...
          // do not update THIS nav-views if its is not the container for the given state
          ...
          // register, update and transition to the new views
...
function($timeout, $controller, $ionicBind) {
  return {
    ...
    controller: function() {},
    compile: function(element, attr) {
      element.addClass('scroll-views ionic-scroll');
 ...
 *
 * Each ionTab has its own views history.
 *
 * @usage
 * ```html
 * <ion-tab
    ...
    compile: function(element, attr) {
      ...
      if (childElementCount) {
        if (tabContentEle.children[0].tagName === 'ION-NAV-VIEW') {
          // get the name if it's a nav-views
          navViewName = tabContentEle.children[0].getAttribute('name');
          tabContentEle.children[0].classList.add('views-container');
          isNavView = true;
        }
      ...
      return function link($scope, $element, $attr, ctrls) {
        ...
        $scope.$on('$destroy', function() {
          if (!$scope.$tabsDestroy) {
            ...
            // is being destroyed, causing unnecessary views loads and transitions
 ...
 *
 * @description
 * A container for views content and any navigational and header bar information. When a views
 * enters and exists its parent {@link ionic.directive:ionNavView}, the views also emits views
 * information, such as its title, whether the back button should show or not, whether the
 * corresponding {@link ionic.directive:ionNavBar} should show or not, which transition the views
 * should use to animate, and which direction to animate.
 *
 * *Views are cached to improve performance.* When a views is navigated away from, its element is
 * left in the DOM, and its scope is disconnected from the `$watch` cycle. When navigating to a
 * views that is already cached, its scope is reconnected, and the existing element, which was
 * left in the DOM, becomes active again. This can be disabled, or the maximum number of cached
 * views changed in {@link ionic.provider:$ionicConfigProvider}, in the views's `$state` configuration, or
 * as an attribute on the views itself (see below).
 *
 * @usage
 ...
 * affect your controller logic. To know when a views has entered or left, events
 * have been added that are emitted from the views's scope. These events also
 * contain data about the views, such as the title and whether the back button should
 ...
 *   <td>The views has loaded. This event only happens once per
 * views being created and added to the DOM. If a views leaves but is cached,
 * then this event will not fire again on a subsequent viewing. The loaded event
 * is good place to put your setup code for the views; however, it is not the
 * recommended event to listen to when a views becomes active.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.enter</code></td>
 *   <td>The views has fully entered and is now the active views.
 * This event will fire, whether it was the first load or a cached views.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.leave</code></td>
 *   <td>The views has finished leaving and is no longer the
 * active views. This event will fire, whether it is cached or destroyed.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.beforeEnter</code></td>
 *   <td>The views is about to enter and become the active views.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.beforeLeave</code></td>
 *   <td>The views is about to leave and no longer be the active views.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.afterEnter</code></td>
 *   <td>The views has fully entered and is now the active views.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.afterLeave</code></td>
 *   <td>The views has finished leaving and is no longer the active views.</td>
 *  </tr>
 *  <tr>
 *   <td><code>$ionicView.unloaded</code></td>
 *   <td>The views's controller has been destroyed and its element has been
 ...
 * either an individual views basis, or by global configuration. Please see the
 * _Caching_ section in {@link ionic.directive:ionNavView} for more info.
 *
 * @param {string=} views-title A text-only title to display on the parent {@link ionic.directive:ionNavBar}.
 * For an HTML title, such as an image, see {@link ionic.directive:ionNavTitle} instead.
 * @param {boolean=} cache-views If this views should be allowed to be cached or not.
