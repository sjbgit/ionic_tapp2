 ...
 * into views.  In order for this to work, any focusable elements must be within
 * a [Scroll View](http://ionicframework.com/docs/api/directive/ionScroll/)
 * or a directive such as [Content](http://ionicframework.com/docs/api/directive/ionContent/)
 * that has a Scroll View.
 *
 * It will also attempt to prevent the native overflow scrolling on focus,
 * which can cause layout issues such as pushing headers up and out of views.
 *
 ...
 *
 * - You can configure the behavior of the web views when the keyboard shows by setting
 ...
 *   out of views on input focus, try setting `cordova.plugins.Keyboard.disableScroll(true)`.
 *   This does **not** disable scrolling in the Ionic scroll views, rather it
...
/**
 * The scroll views containing the currently focused input.
 */
var scrollView;
...
function keyboardFocusIn(e) {
  ...
  // scroll views is positioned correctly
  ...
  // attempt to prevent browser from natively scrolling input into views while
  ...
  // if the keyboard is already open, go ahead and scroll the input into views
  // if necessary
 ...
 * scrolling an input into views when a user taps the keyboard while we are
 * scrolling the input into views ourselves with JS.
 */
 ...
 * elements outside the scroll views while the keyboard is open.
 */
 ...
 * delay as essentially a timeout so that scroll views adjustments happen after
...
/**
 * On keyboard close sets keyboard state to closed, resets the scroll views,
 ...
 * indicating the keyboard is open and tells the scroll views to resize and
 * the currently focused input into views if necessary.
 */
function keyboardShow() {
  ...
  // send event so the scroll views adjusts
...
/**
 * ionic.views.Scroll
 * A powerful scroll views with support for bouncing, pull to refresh, and paging.
 * @param   {Object}        options options for the scroll views
 * @class A scroll views system
...
ionic.views.Scroll = ionic.views.View.inherit({
  initialize: function(options) {
    ...
    self.options = {
      ...
      /** The velocity required to make the scroll views "slide" after touchend */
      decelVelocityThreshold: 4,

      /** The velocity required to make the scroll views "slide" after touchend when using paging */
  ...
  */
  ...
  __initEventHandlers: function() {
    ...
    // save height when scroll views is shrunk so we don't need to reflow
    var scrollViewOffsetHeight;

    /**
     * Shrink the scroll views when the keyboard is up if necessary and if the
     * focused input is below the bottom of the shrunk scroll views, scroll it
     * into views.
     */
    self.scrollChildIntoView = function(e) {
     ...
     /*
      *  _______
      * |---A---| <- top of scroll views
      * |       |
      * |---B---| <- keyboard
      * |   C   | <- input
      * |---D---| <- initial bottom of scroll views
      ...
      */
      if (!alreadyShrunk) {
        ...
        if ( ionic.Platform.isIOS() || ionic.Platform.isFullScreen || isInsetModal ) {
          // if there are things below the scroll views account for them and
       ...
       * |--B-D--| <- keyboard, bottom of scroll views
       * |   C   | <- input
       * |       |
       * |___E___| <- bottom of viewport
       *
       ...
       *  is the viewport height, not 0)
       */
      // if the element is positioned under the keyboard scroll it into views
      if (e.detail.isElementUnderKeyboard) {
        ...
        ionic.requestAnimationFrame(function(){
          ...
          // middle of the input we want to scroll into views
          // C
          var inputMidpoint = ((e.detail.elementBottom + e.detail.elementTop) / 2);

          // distance from middle of input to the bottom of the scroll views
          // C - D                                C               D
    ...
    // input and returning to a previously resized scroll views in an ion-content.
    // Since we can only resize scroll views that are currently visible, just resize
    // the current scroll views when the keyboard is closed.
    ...
    self.touchMove = function(e) {
      ...
      if (self.startCoordinates) {
        ...
        if ( self.__enableScrollY && Math.abs(self.startCoordinates.y - currentCoordinates.y) > 10 ) {
          // user scrolled the entire views on the y axis
  ...
  */
  ...
  /**
   * If the scroll views isn't sized correctly on start, wait until we have at least some size
   */
...
'use strict';
  ...
  /**
   * The side menu views handles one of the side menu's in a Side Menu Controller
   * configuration.
